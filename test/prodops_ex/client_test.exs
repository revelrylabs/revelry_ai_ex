defmodule ProdopsEx.ClientTest do
  ...
  describe "handle_response/1" do
    test "it should respond with success if HTTP status code 200 and the response is a JSON" do
      res =
        ProdopsEx.Client.handle_response({:ok, %HTTPoison.Response{body: {:ok, %{"key" => "value"}}, status_code: 200}})
    ...
    test "it should respond with error if HTTP status code is not 200" do
      res =
        ProdopsEx.Client.handle_response(
    ...
    test "it should handle non-JSON responses correctly" do
      res =
        ProdopsEx.Client.handle_response({:ok, %HTTPoison.Response{body: "Service Unavailable", status_code: 503}})
    ...
    test "it should handle HTTPoison errors" do
      res =
        ProdopsEx.Client.handle_response({:error, %HTTPoison.Error{reason: :timeout}})
